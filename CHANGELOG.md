...
- User-declared `var`-typed locals are now initialized to `ZVAL_NULL` instead of `ZVAL_UNDEF` at the function prologue. Reading an unassigned `var x;` now yields `null` (matching PHP semantics for an unset local) instead of an undef zval that PHP 8.x surfaces as `Undefined variable` at the receiver. Compiler-generated temporaries, parameter holders, and native-string `_zv` companion zvals are unaffected. [#1875](https://github.com/zephir-lang/zephir/issues/1875)
