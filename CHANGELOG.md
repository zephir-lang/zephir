...
### Fixed
- Fixed double-quoted string escapes not matching PHP: `\u{XXXX}` Unicode codepoints are now UTF-8 encoded (previously emitted literally), and `\xHH` no longer absorbs following hex digits (e.g. `"\x41BC"` is now `"ABC"`) [#2030](https://github.com/zephir-lang/zephir/issues/2030)
