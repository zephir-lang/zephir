...
static PHP_GINIT_FUNCTION(%PROJECT_LOWER%)
{
	...
	%PROJECT_LOWER%_globals->end_memory = start + num_preallocated_frames;
...
static PHP_GSHUTDOWN_FUNCTION(%PROJECT_LOWER%)
{
	...
	for (i = 0; i < num_preallocated_frames; ++i) {
		pefree(%PROJECT_LOWER%_globals->start_memory[i].hash_addresses, 1);
		pefree(%PROJECT_LOWER%_globals->start_memory[i].addresses, 1);
	}

	pefree(%PROJECT_LOWER%_globals->start_memory, 1);
	%PROJECT_LOWER%_globals->start_memory = NULL;

	zend_hash_destroy(%PROJECT_LOWER%_globals->fcache);
	pefree(%PROJECT_LOWER%_globals->fcache, 1);
	%PROJECT_LOWER%_globals->fcache = NULL;
}
