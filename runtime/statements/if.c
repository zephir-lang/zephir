...
int zephir_statement_if(zephir_context *context, zval *statement TSRMLS_DC)
{
	...
	switch (compiled_expr->type) {
		...
		case ZEPHIR_T_INTEGER:
			condition = LLVMBuildICmp(context->builder, LLVMIntNE, compiled_expr->value, LLVMConstInt(LLVMInt32Type(), 0, 0), "ifcond");
			break;
		...
		case ZEPHIR_T_VARIABLE:
			...
			switch (compiled_expr->variable->type) {
				...
				case ZEPHIR_T_TYPE_VAR:
					condition = LLVMBuildICmp(context->builder, LLVMIntNE, zephir_build_zend_is_true(context, compiled_expr->variable->value_ref), LLVMConstInt(LLVMInt32Type(), 0, 0), "ifcond");
					break;
