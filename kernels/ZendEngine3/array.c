		...
		switch (types[i]) {
			...
			case 'l':
				...
				if (zephir_array_isset_long_fetch(&fetched, &pzv, ll, 1)) {
					if (Z_TYPE(fetched) == IS_ARRAY) {
						if (i == (types_length - 1)) {
							zephir_array_update_long(&pzv, ll, value, 0 ZEPHIR_DEBUG_PARAMS_DUMMY);
							p = Z_ARRVAL(pzv);
						} else {
							if (Z_REFCOUNT_P(&fetched) > 1) {
								ZVAL_ARR(&fetched, zend_array_dup(Z_ARR_P(&fetched)));
			...
			case 'z':
				...
				if (zephir_array_isset_fetch(&fetched, &pzv, item, 1)) {
					if (Z_TYPE(fetched) == IS_ARRAY) {
						if (i == (types_length - 1)) {
							zephir_array_update_zval(&pzv, item, value, 0);
							p = Z_ARRVAL(pzv);
						} else {
							if (Z_REFCOUNT_P(&fetched) > 1) {
								ZVAL_ARR(&fetched, zend_array_dup(Z_ARR_P(&fetched)));
			...
			case 'a':
				zephir_array_append(&pzv, value, 0 ZEPHIR_DEBUG_PARAMS_DUMMY);
				p = Z_ARRVAL(pzv);
				break;
		}
