...
PHP_METHOD(Stub_Reflection, getReflectionClass)
{
	...
	ZEPHIR_CALL_METHOD(NULL, &r, "__construct", NULL, 103, this_ptr);
...
PHP_METHOD(Stub_Reflection, getReflectionFunction)
{
	...
	ZEPHIR_INIT_VAR(&_0);
	ZVAL_STRING(&_0, "abs");
	ZEPHIR_CALL_METHOD(NULL, &r, "__construct", NULL, 104, &_0);
	zephir_check_call_status();
	RETURN_CCTOR(&r);
}
...
PHP_METHOD(Stub_Reflection, setReflectionParameter)
{
	...
	if (UNEXPECTED(!_zephir_prop_0)) {
		_zephir_prop_0 = zend_string_init("reflectionParameter", 19, 1);
	}
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 107, parameter);
