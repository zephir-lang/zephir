...
PHP_METHOD(Stub_Bench_Foo, readProp)
{
	...
	if (ZEPHIR_GE_LONG(&_2, _1)) {
		while (1) {
			...
			ZEPHIR_INIT_NVAR(&i);
			ZVAL_LONG(&i, _1);
			zephir_read_property_cached(&x, this_ptr, _zephir_prop_0, 22, PH_NOISY_CC | PH_READONLY);
		}
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Stub_Bench_Foo, writeProp)
{
	...
	if (ZEPHIR_GE_LONG(&_2, _1)) {
		while (1) {
			...
			ZVAL_UNDEF(&_3$$3);
			ZVAL_LONG(&_3$$3, 0);
			zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 22, &_3$$3);
		}
	}
	ZEPHIR_MM_RESTORE();
}
...
PHP_METHOD(Stub_Bench_Foo, assignAddProp)
{
	...
	if (ZEPHIR_GE_LONG(&_2, _1)) {
		while (1) {
			...
			ZVAL_LONG(&_4$$3, 2);
			zephir_read_property_cached(&_3$$3, this_ptr, _zephir_prop_0, 22, PH_NOISY_CC);
