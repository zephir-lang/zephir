...
PHP_METHOD(Stub_BuiltIn_ArrayMethods, issue733BuiltInJoinSpecialChars)
{
	...
	ZEPHIR_INIT_VAR(&_4);
	ZVAL_STRING(&_4, "\\\\$0");
	ZEPHIR_CALL_FUNCTION(&_5, "preg_replace", &_6, 9, &_3, &_4, b);
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(b, &_5);
	ZEPHIR_INIT_VAR(&_7);
	...
	ZEPHIR_INIT_NVAR(&_3);
	ZVAL_STRING(&_3, "");
	ZEPHIR_RETURN_CALL_FUNCTION("preg_replace", &_6, 9, &_7, &_3, &a_zv);
	zephir_check_call_status();
	RETURN_MM();
}
...
PHP_METHOD(Stub_BuiltIn_ArrayMethods, issue733ReversedOnVar)
{
	...
	zephir_memory_grow_stack(ZEPHIR_METHOD_GLOBALS_PTR, __func__);
	zephir_fetch_params(1, 1, 0, &input);
	ZEPHIR_CALL_FUNCTION(&_0, "array_reverse", NULL, 7, input);
	zephir_check_call_status();
	RETURN_CCTOR(&_0);
}
