...
PHP_METHOD(Stub_Openssl, randomPseudoBytes)
{
	...
	ZVAL_LONG(&_0, length);
	ZEPHIR_RETURN_CALL_FUNCTION("openssl_random_pseudo_bytes", NULL, 84, &_0);
...
PHP_METHOD(Stub_Openssl, randomPseudoBytesLoop)
{
	...
	if (_1 <= _2) {
		while (1) {
			...
			ZVAL_LONG(&i, _1);
			ZVAL_LONG(&_3$$3, 10);
			ZEPHIR_CALL_FUNCTION(&_4$$3, "openssl_random_pseudo_bytes", &_5, 84, &_3$$3);
			zephir_check_call_status();
...
PHP_METHOD(Stub_Openssl, opensslEncrypt)
{
	...
	ZEPHIR_RETURN_CALL_FUNCTION("openssl_encrypt", NULL, 85, &data_zv, &_0, &_1, &_2, &_3);
	zephir_check_call_status();
	RETURN_MM();
}
