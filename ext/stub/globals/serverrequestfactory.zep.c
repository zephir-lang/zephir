...
PHP_METHOD(Stub_Globals_ServerRequestFactory, load)
{
	...
	ZEND_PARSE_PARAMETERS_START(0, 5)
		Z_PARAM_OPTIONAL
		ZEPHIR_Z_PARAM_ARRAY_OR_NULL(server, server_param)
		ZEPHIR_Z_PARAM_ARRAY_OR_NULL(get, get_param)
		ZEPHIR_Z_PARAM_ARRAY_OR_NULL(post, post_param)
		ZEPHIR_Z_PARAM_ARRAY_OR_NULL(cookies, cookies_param)
		ZEPHIR_Z_PARAM_ARRAY_OR_NULL(files, files_param)
	...
	ZEPHIR_CALL_METHOD(&_0, this_ptr, "checknullarray", &_1, 50, &server, &globalServer);
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(&server, &_0);
	ZEPHIR_CALL_METHOD(&_2, this_ptr, "checknullarray", &_1, 50, &files, &globalFiles);
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(&files, &_2);
	ZEPHIR_CALL_METHOD(&_3, this_ptr, "checknullarray", &_1, 50, &cookies, &globalCookies);
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(&cookies, &_3);
	ZEPHIR_CALL_METHOD(&_4, this_ptr, "checknullarray", &_1, 50, &get, &globalGet);
	zephir_check_call_status();
	ZEPHIR_CPY_WRT(&get, &_4);
	ZEPHIR_CALL_METHOD(&_5, this_ptr, "checknullarray", &_1, 50, &post, &globalPost);
	zephir_check_call_status();
...
PHP_METHOD(Stub_Globals_ServerRequestFactory, checkNullArray)
{
	...
	ZEND_PARSE_PARAMETERS_START(2, 2)
		Z_PARAM_ZVAL(source)
		ZEPHIR_Z_PARAM_ARRAY(super, super_param)
	ZEND_PARSE_PARAMETERS_END();
	ZEPHIR_METHOD_GLOBALS_PTR = pecalloc(1, sizeof(zephir_method_globals), 0);
	zephir_memory_grow_stack(ZEPHIR_METHOD_GLOBALS_PTR, __func__);
