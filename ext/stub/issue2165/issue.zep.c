...
PHP_METHOD(Stub_Issue2165_Issue, build)
{
	...
	ZVAL_BOOL(&_0, 1);
	ZEPHIR_CALL_METHOD(NULL, return_value, "__construct", NULL, 61, &a, &_0);
...
PHP_METHOD(Stub_Issue2165_Issue, __construct)
{
	...
	ZEPHIR_CALL_FUNCTION(&_0, "current", NULL, 56, &a);
	zephir_check_call_status();
	...
	if (validate) {
		...
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&a), _4$$3, _5$$3, _3$$3)
		{
			...
			if (UNEXPECTED(zephir_fast_count_int(&rowA) != n)) {
				...
				ZVAL_LONG(&_7$$5, n);
				ZEPHIR_CALL_FUNCTION(&_8$$5, "strval", &_9, 63, &_7$$5);
				zephir_check_call_status();
			...
			if (Z_TYPE_P(_13$$4) == IS_ARRAY) {
				ZEND_HASH_FOREACH_VAL(Z_ARRVAL_P(_13$$4), _15$$4)
				{
					...
					if (_16$$6) {
						ZEPHIR_CALL_FUNCTION(&_17$$6, "is_float", &_18, 65, &valueA);
						zephir_check_call_status();
				...
				while (1) {
					...
					zephir_check_call_status();
						...
						if (_24$$8) {
							ZEPHIR_CALL_FUNCTION(&_25$$8, "is_float", &_18, 65, &valueA);
							zephir_check_call_status();
			...
			ZEPHIR_INIT_NVAR(&valueA);
			ZEPHIR_CALL_FUNCTION(&_29$$4, "array_values", &_2, 62, &rowA);
			zephir_check_call_status();
...
PHP_METHOD(Stub_Issue2165_Issue, fill)
{
	...
	if (_0) {
		ZEPHIR_CALL_FUNCTION(&_1, "is_float", NULL, 65, value);
		zephir_check_call_status();
	...
	if (UNEXPECTED(m < 1)) {
		...
		ZVAL_LONG(&_6$$4, m);
		ZEPHIR_CALL_FUNCTION(&_7$$4, "strval", &_8, 63, &_6$$4);
		zephir_check_call_status();
	...
	if (UNEXPECTED(n < 1)) {
		...
		ZVAL_LONG(&_11$$5, n);
		ZEPHIR_CALL_FUNCTION(&_12$$5, "strval", &_8, 63, &_11$$5);
		zephir_check_call_status();
