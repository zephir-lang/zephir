...
PHP_METHOD(Stub_Fibonnaci, fibArray) {
	...
	while (1) {
		...
		k = (i - 1);
		j = (i - 2);
		zephir_array_fetch_long(&a, &fib, k, PH_NOISY | PH_READONLY, "stub/fibonnaci.zep", 51);
		zephir_array_fetch_long(&b, &fib, j, PH_NOISY | PH_READONLY, "stub/fibonnaci.zep", 52);
		ZEPHIR_INIT_NVAR(&c);
...
PHP_METHOD(Stub_Fibonnaci, fibonacciRecursive) {
	...
	if (n == 0) {
		...
		if (n == 1) {
			...
			ZVAL_LONG(&_1$$6, (n - 1));
			ZEPHIR_CALL_METHOD(&_0$$6, this_ptr, "fibonaccirecursive", &_2, 37, &_1$$6);
			zephir_check_call_status();
			ZVAL_LONG(&_1$$6, (n - 2));
			ZEPHIR_CALL_METHOD(&_3$$6, this_ptr, "fibonaccirecursive", &_2, 37, &_1$$6);
			zephir_check_call_status();
...
PHP_METHOD(Stub_Fibonnaci, fibonacciFinalRecursive) {
	...
	if (n == 0) {
		...
		if (n == 1) {
			...
			ZVAL_LONG(&_1$$6, (n - 1));
			ZEPHIR_CALL_METHOD(&_0$$6, this_ptr, "fibonaccifinalrecursive", &_2, 38, &_1$$6);
			zephir_check_call_status();
			ZVAL_LONG(&_1$$6, (n - 2));
			ZEPHIR_CALL_METHOD(&_3$$6, this_ptr, "fibonaccifinalrecursive", &_2, 38, &_1$$6);
			zephir_check_call_status();
			zephir_add_function(return_value, &_0$$6, &_3$$6);
			RETURN_MM();
		}
