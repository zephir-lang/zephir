...
PHP_METHOD(Stub_Oo_PropertyAccess, __construct)
{
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_3, 74, &test);
	zephir_read_property_cached(&_9, this_ptr, _zephir_prop_3, 74, PH_NOISY_CC | PH_READONLY);
	zephir_read_property_cached(&_10, &_9, _zephir_prop_1, 0, PH_NOISY_CC | PH_READONLY);
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_4, 75, &_10);
	zephir_memory_observe(&test1);
	zephir_read_property_cached(&test1, this_ptr, _zephir_prop_3, 74, PH_NOISY_CC);
	ZEPHIR_MM_RESTORE();
}
