...
PHP_METHOD(Stub_Oo_OoDestruct, __construct)
{
	...
	if ((zephir_file_exists(&_1) == SUCCESS)) {
		...
		ZEPHIR_CALL_FUNCTION(&_3$$4, "realpath", NULL, 75, &_2$$4);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(&imageinfo, "getimagesize", NULL, 76, &_4$$4);
		zephir_check_call_status();
		...
		ZEPHIR_CALL_FUNCTION(NULL, "imagealphablending", NULL, 77, &_35$$13, &__$true);
		zephir_check_call_status();
...
PHP_METHOD(Stub_Oo_OoDestruct, getVersion)
{
	...
	ZEPHIR_CALL_FUNCTION(&_4, "defined", NULL, 78, &_3);
	zephir_check_call_status();
	if (zephir_is_true(&_4)) {
		ZEPHIR_INIT_NVAR(&version);
		ZEPHIR_GET_CONSTANT(&version, "GD_VERSION");
	} else {
		ZEPHIR_CALL_FUNCTION(&info, "gd_info", NULL, 79);
		zephir_check_call_status();
