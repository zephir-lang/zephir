...
PHP_METHOD(Stub_Properties_PublicProperties, setSomeGetterSetterArray)
{
	...
	if (UNEXPECTED(!_zephir_prop_0)) {
		_zephir_prop_0 = zend_string_init("someGetterSetterArray", 21, 1);
	}
	...
	zephir_update_property_zval_cached(this_ptr, _zephir_prop_0, 3, someGetterSetterArray);
	RETURN_THISW();
}
...
PHP_METHOD(Stub_Properties_PublicProperties, test394Issue)
{
	...
	static zend_string *_zephir_prop_0 = NULL;
	if (UNEXPECTED(!_zephir_prop_0)) {
		_zephir_prop_0 = zend_string_init("someTrue", 8, 1);
	}
	ZEPHIR_METHOD_GLOBALS_PTR = pecalloc(1, sizeof(zephir_method_globals), 0);
	zephir_memory_grow_stack(ZEPHIR_METHOD_GLOBALS_PTR, __func__);

	zephir_memory_observe(&_0);
	zephir_read_property_cached(&_0, this_ptr, _zephir_prop_0, 4, PH_NOISY_CC);
	RETURN_CCTOR(&_0);
}
...
zend_object *zephir_init_properties_Stub_Properties_PublicProperties(zend_class_entry *class_type)
{
	...
	
	{
		...
		if (Z_TYPE_P(&_2) == IS_NULL) {
			...
			zephir_update_property_zval_ex(this_ptr, ZEND_STRL("someArrayWithSubArray"), &_3$$4);
		}
		...
		if (Z_TYPE_P(&_5) == IS_NULL) {
			...
			zephir_update_property_zval_ex(this_ptr, ZEND_STRL("someArrayWithSpecifyKeys"), &_6$$5);
		}
		...
		if (Z_TYPE_P(&_7) == IS_NULL) {
			...
			ZVAL_LONG(&_9$$6, 4);
			zephir_array_fast_append(&_8$$6, &_9$$6);
			zephir_update_property_zval_ex(this_ptr, ZEND_STRL("someArray"), &_8$$6);
		}
