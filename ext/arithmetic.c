...
PHP_METHOD(Test_Arithmetic, complex18Sum) {
	...
	b = (2.0) ? 1 : 0;
	ZEPHIR_SINIT_VAR(d);
	ZVAL_LONG(&d, zephir_get_numberval(&a) + b);
	c = (double) (zephir_get_numberval(&a) + b + zephir_get_intval(&d));
	RETURN_DOUBLE(c);
...
PHP_METHOD(Test_Arithmetic, complex19Sum) {
	...
	c = (double) (zephir_get_numberval(_0) + b);
	RETURN_MM_DOUBLE(c);
...
PHP_METHOD(Test_Arithmetic, complex23Sum) {
	...
	ZVAL_LONG(&a, 1);
	b = (2.0) ? 1 : 0;
	d = (double) (zephir_get_numberval(&a) + b);
	RETURN_DOUBLE(d);
