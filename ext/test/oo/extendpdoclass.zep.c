...
PHP_METHOD(Test_Oo_ExtendPdoClass, __construct) {
	...
	if (!username_param) {
		ZEPHIR_MM_ZVAL_STRING(&username, "");
	} else {
		zephir_get_strval(&username, username_param);
	}
	if (!password_param) {
		ZEPHIR_MM_ZVAL_STRING(&password, "");
	} else {
	...
	ZEPHIR_MM_ZVAL_STRING(&_1, "Test\\PdoStatement");
	zephir_array_fast_append(&_0, &_1);
	zephir_array_update_long(attrs, 13, &_0, PH_SEPARATE ZEPHIR_DEBUG_PARAMS_DUMMY);
