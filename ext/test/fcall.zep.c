...
PHP_METHOD(Test_Fcall, testCall3) {
	...
	if (zephir_is_true(&handle)) {
		while (1) {
			ZVAL_LONG(&_3$$4, 4096);
			ZEPHIR_CALL_FUNCTION(&buffer, "fgets", &_4, 31, &handle, &_3$$4);
			zephir_check_call_status();
...
PHP_METHOD(Test_Fcall, testArrayFill) {
	...
	ZEPHIR_INIT_VAR(&_2);
	ZVAL_STRING(&_2, "?");
	ZEPHIR_CALL_FUNCTION(&v1, "array_fill", &_3, 35, &_0, &_1, &_2);
	zephir_check_call_status();
	ZVAL_LONG(&_0, 0);
	ZVAL_LONG(&_1, 6);
	ZEPHIR_INIT_NVAR(&_2);
	ZVAL_STRING(&_2, "?");
	ZEPHIR_CALL_FUNCTION(&v2, "array_fill", &_3, 35, &_0, &_1, &_2);
	zephir_check_call_status();
	zephir_create_array(return_value, 2, 0 TSRMLS_CC);
...
PHP_FUNCTION(g_test_zephir_global_method_test) {
	...
	ZEPHIR_RETURN_CALL_METHOD(&_0, "teststrtokvarbyslash", NULL, 36, str);
	zephir_check_call_status();
	RETURN_MM();
...
PHP_FUNCTION(f_Test_zephir_namespaced_method_test) {
	...
	ZVAL_LONG(&_1, 5);
	ZEPHIR_RETURN_CALL_METHOD(&_0, "testcall5", NULL, 37, str, &_1);
	zephir_check_call_status();
	RETURN_MM();
