...
PHP_METHOD(Test_Quantum, harmos) {
	...
	i = 0;
	array_init(&psr);
	ZEPHIR_MM_ADD_ENTRY(&psr);
	array_init(&psi);
	ZEPHIR_MM_ADD_ENTRY(&psi);
	array_init(&p2);
	ZEPHIR_MM_ADD_ENTRY(&p2);
	array_init(&v);
	ZEPHIR_MM_ADD_ENTRY(&v);
	ZEPHIR_MM_ZVAL_STRING(&_0, "harmos.txt");
	ZEPHIR_MM_ZVAL_STRING(&_1, "w");
	...
	while (1) {
		...
		zephir_array_update_long(&psr, i, &_8$$4, PH_SEPARATE ZEPHIR_DEBUG_PARAMS_DUMMY);
		zephir_create_array(&_10$$4, 1, 0);
		ZEPHIR_MM_ADD_ENTRY(&_10$$4);
		ZVAL_DOUBLE(&_9$$4, item_psi);
		zephir_array_fast_append(&_10$$4, &_9$$4);
		zephir_array_update_long(&psi, i, &_10$$4, PH_SEPARATE ZEPHIR_DEBUG_PARAMS_DUMMY);
		zephir_create_array(&_11$$4, 1, 0);
		ZEPHIR_MM_ADD_ENTRY(&_11$$4);
		ZVAL_DOUBLE(&_9$$4, (double) ((double) (5.0 * x) * x));
		zephir_array_fast_append(&_11$$4, &_9$$4);
		zephir_array_update_long(&v, i, &_11$$4, PH_SEPARATE ZEPHIR_DEBUG_PARAMS_DUMMY);
		x = (x + dx);
		i++;
	}
	i = 1;
		...
		while (1) {
			...
			if (_16$$5 <= _17$$5) {
				while (1) {
					...
					zephir_array_update_long(&p2, i, &_28$$6, PH_SEPARATE ZEPHIR_DEBUG_PARAMS_DUMMY);
				}
			}
			_36$$5 = (751 - 1);
			_35$$5 = 1;
			...
			if (_50$$5) {
				...
				while (1) {
					...
					ZEPHIR_MM_ZVAL_STRING(&_52$$9, "%16.8lf %16.8lf %16.8lf \n");
					ZVAL_DOUBLE(&_53$$9, ((double) i * dx));
					ZVAL_DOUBLE(&_54$$9, ((double) n * dt));
					ZEPHIR_CALL_FUNCTION(NULL, "fprintf", &_55, 61, &fp, &_52$$9, &_53$$9, &_54$$9, &_51$$9);
					zephir_check_call_status();
					i = (i + 10);
				}
				ZEPHIR_MM_ZVAL_STRING(&_56$$8, "\n");
				ZEPHIR_CALL_FUNCTION(NULL, "fprintf", &_55, 61, &fp, &_56$$8);
				zephir_check_call_status();
			}
			j = 1;
			while (1) {
