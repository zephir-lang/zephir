	...
	while (1) {
		...
		if (_6$$4 <= _7$$4) {
			while (1) {
				...
				i = _6$$4;
				zephir_array_fetch_long(&_8$$7, &perm1, i, PH_NOISY | PH_READONLY, "test/fannkuch.zep", 43);
		...
		k = 0;
		while (1) {
			zephir_array_fetch_long(&_9$$4, &perm, 0, PH_NOISY | PH_READONLY, "test/fannkuch.zep", 48);
			if (!(!ZEPHIR_IS_LONG(&_9$$4, 0))) {
				break;
			}
			ZEPHIR_OBS_NVAR(&_10$$8);
			zephir_array_fetch_long(&_10$$8, &perm, 0, PH_NOISY, "test/fannkuch.zep", 49);
			...
			if (_12$$8 <= _13$$8) {
				while (1) {
					...
					i = _12$$8;
					zephir_array_fetch_long(&temp, &perm, i, PH_NOISY | PH_READONLY, "test/fannkuch.zep", 51);
					zephir_array_fetch_long(&_14$$9, &perm, (k - i), PH_NOISY | PH_READONLY, "test/fannkuch.zep", 51);
		...
		while (1) {
			...
			zephir_array_fetch_long(&perm0, &perm1, 0, PH_NOISY | PH_READONLY, "test/fannkuch.zep", 75);
			i = 0;
			while (1) {
				if (!(i < r)) {
					break;
				...
				j = (i + 1);
				zephir_array_fetch_long(&_16$$15, &perm1, j, PH_NOISY | PH_READONLY, "test/fannkuch.zep", 78);
			...
			zephir_array_fetch_long(&_17$$13, &count, r, PH_NOISY | PH_READONLY, "test/fannkuch.zep", 82);
			...
			zephir_array_fetch_long(&_19$$13, &count, r, PH_NOISY | PH_READONLY, "test/fannkuch.zep", 83);
