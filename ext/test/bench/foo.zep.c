...
PHP_METHOD(Test_Bench_Foo, readStatic) {
	...
	if (ZEPHIR_GE_LONG(&_2, _1)) {
		while (1) {
			...
			ZVAL_LONG(&i, _1);
			zephir_read_static_property_ce(&x, test_bench_foo_ce, SL("a"), PH_NOISY_CC);
		}
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Test_Bench_Foo, writeStatic) {
	...
	if (ZEPHIR_GE_LONG(&_2, _1)) {
		while (1) {
			...
			ZVAL_LONG(&_3$$3, 0);
			zephir_update_static_property(test_bench_foo_ce, ZEND_STRL("a"), &_3$$3);
		}
	}
	ZEPHIR_MM_RESTORE();
...
PHP_METHOD(Test_Bench_Foo, emptyStatic) {
	...
	if (ZEPHIR_GE_LONG(&_2, _1)) {
		while (1) {
			...
			ZVAL_LONG(&i, _1);
			zephir_read_static_property_ce(&_3$$3, test_bench_foo_ce, SL("a"), PH_NOISY_CC);
