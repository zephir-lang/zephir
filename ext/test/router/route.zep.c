...
PHP_METHOD(Test_Router_Route, compilePattern) {
	...
	if (zephir_memnstr_str(pattern, SL(":"), "test/router/route.zep", 56)) {
		ZEPHIR_MM_ZVAL_STRING(&idPattern, "/([a-zA-Z0-9\\_\\-]+)");
		if (zephir_memnstr_str(pattern, SL("/:module"), "test/router/route.zep", 62)) {
			ZEPHIR_MM_ZVAL_STRING(&_1$$4, "/:module");
		...
		if (zephir_memnstr_str(pattern, SL("/:controller"), "test/router/route.zep", 67)) {
			ZEPHIR_MM_ZVAL_STRING(&_3$$5, "/:controller");
		...
		if (zephir_memnstr_str(pattern, SL("/:namespace"), "test/router/route.zep", 72)) {
			ZEPHIR_MM_ZVAL_STRING(&_5$$6, "/:namespace");
		...
		if (zephir_memnstr_str(pattern, SL("/:action"), "test/router/route.zep", 77)) {
			ZEPHIR_MM_ZVAL_STRING(&_7$$7, "/:action");
		...
		if (zephir_memnstr_str(pattern, SL("/:params"), "test/router/route.zep", 82)) {
			ZEPHIR_MM_ZVAL_STRING(&_9$$8, "/:params");
			ZEPHIR_MM_ZVAL_STRING(&_10$$8, "(/.*)*");
		...
		if (zephir_memnstr_str(pattern, SL("/:int"), "test/router/route.zep", 87)) {
			ZEPHIR_MM_ZVAL_STRING(&_12$$9, "/:int");
			ZEPHIR_MM_ZVAL_STRING(&_13$$9, "/([0-9]+)");
...
PHP_METHOD(Test_Router_Route, extractNamedParams) {
	...
	array_init(&matches);
	ZEPHIR_MM_ADD_ENTRY(&matches);
	ZEPHIR_MM_ZVAL_STRING(&route, "");
	for (_0 = 0; _0 < Z_STRLEN_P(&pattern); _0++) {
			...
			if (ch == '{') {
				...
				if (ch == '}') {
					...
					if (intermediate > 0) {
						if (bracketCount == 0) {
							numberMatches++;
							ZEPHIR_MM_ZVAL_STRING(&variable, "");
							ZEPHIR_MM_ZVAL_STRING(&regexp, "");
							...
							if (!(notValid)) {
								...
								if (_25$$18) {
									...
									if (foundPattern != 2) {
										ZEPHIR_MM_CONCAT_SELF_STR(&route, "(", sizeof("(")-1);
										ZEPHIR_MM_CONCAT_SELF(&route, &regexp);
										ZEPHIR_MM_CONCAT_SELF_STR(&route, ")", sizeof(")")-1);
									} else {
										ZEPHIR_MM_CONCAT_SELF(&route, &regexp);
									}
									ZVAL_LONG(&_27$$19, tmp);
									zephir_array_update_zval(&matches, &variable, &_27$$19, PH_SEPARATE);
								} else {
									ZEPHIR_MM_CONCAT_SELF_STR(&route, "([^/]*)", sizeof("([^/]*)")-1);
									ZVAL_LONG(&_28$$28, tmp);
									zephir_array_update_zval(&matches, &item, &_28$$28, PH_SEPARATE);
								}
							} else {
								ZEPHIR_CONCAT_SVS(&_29$$29, "{", &item, "}");
								ZEPHIR_MM_ADD_ENTRY(&_29$$29);
								ZEPHIR_MM_CONCAT_SELF(&route, &_29$$29);
							}
							continue;
						}
					}
		...
		if (bracketCount > 0) {
			...
			ZEPHIR_MM_CONCAT_SELF_CHAR(&route, ch);
		}
	}
...
PHP_METHOD(Test_Router_Route, reConfigure) {
	...
	if (Z_TYPE_P(paths) != IS_NULL) {
		if (Z_TYPE_P(paths) == IS_STRING) {
			...
			ZEPHIR_MM_ADD_ENTRY(&routePaths);
			if (Z_TYPE_P(&moduleName) != IS_NULL) {
				zephir_array_update_string(&routePaths, SL("module"), &moduleName, PH_SEPARATE);
			}
			if (Z_TYPE_P(&controllerName) != IS_NULL) {
				if (zephir_memnstr_str(&controllerName, SL("\\"), "test/router/route.zep", 310)) {
					...
					if (zephir_is_true(&namespaceName)) {
						zephir_array_update_string(&routePaths, SL("namespace"), &namespaceName, PH_SEPARATE);
					}
				} else {
					ZEPHIR_CPY_WRT(&realClassName, &controllerName);
				}
				zephir_uncamelize(&_1$$10, &realClassName, NULL  );
				zephir_array_update_string(&routePaths, SL("controller"), &_1$$10, PH_SEPARATE);
			}
			if (Z_TYPE_P(&actionName) != IS_NULL) {
				zephir_array_update_string(&routePaths, SL("action"), &actionName, PH_SEPARATE);
			}
		} else {
...
PHP_METHOD(Test_Router_Route, getName) {
	...
	RETURN_MM_MEMBER(getThis(), "_name");
...
PHP_METHOD(Test_Router_Route, getBeforeMatch) {
	...
	RETURN_MM_MEMBER(getThis(), "_beforeMatch");
...
PHP_METHOD(Test_Router_Route, getPattern) {
	...
	RETURN_MM_MEMBER(getThis(), "_pattern");
...
PHP_METHOD(Test_Router_Route, getCompiledPattern) {
	...
	RETURN_MM_MEMBER(getThis(), "_compiledPattern");
...
PHP_METHOD(Test_Router_Route, getPaths) {
	...
	RETURN_MM_MEMBER(getThis(), "_paths");
...
PHP_METHOD(Test_Router_Route, getReversedPaths) {
	...
	if (Z_TYPE_P(&_0) == IS_ARRAY) {
		ZEND_HASH_FOREACH_KEY_VAL(Z_ARRVAL_P(&_0), _3, _4, _1)
		{
			...
			ZEPHIR_CPY_WRT(&position, _1);
			zephir_array_update_zval(&reversed, &position, &path, PH_SEPARATE);
...
PHP_METHOD(Test_Router_Route, getHttpMethods) {
	...
	RETURN_MM_MEMBER(getThis(), "_methods");
...
PHP_METHOD(Test_Router_Route, getHostname) {
	...
	RETURN_MM_MEMBER(getThis(), "_hostname");
...
PHP_METHOD(Test_Router_Route, getConverters) {
	...
	RETURN_MM_MEMBER(getThis(), "_converters");
