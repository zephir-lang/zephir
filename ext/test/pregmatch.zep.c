...
PHP_METHOD(Test_Pregmatch, testWithoutReturnAndMatches) {
	...
	ZEPHIR_MM_ZVAL_STRING(&pattern, "/def$/");
	ZEPHIR_MM_ZVAL_STRING(&subject, "abcdef");
...
PHP_METHOD(Test_Pregmatch, testMatchAll) {
	...
	array_init(&matches);
	ZEPHIR_MM_ADD_ENTRY(&matches);
	ZEPHIR_MM_ZVAL_STRING(&text, "test1,test2");
	ZEPHIR_MM_ZVAL_STRING(&_0, "/(test[0-9]+)/");
	ZEPHIR_MM_ZVAL_STRING(&_2, "/(test[0-9]+)/");
