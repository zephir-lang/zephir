...
PHP_METHOD(Test_Properties_PropertyArray, testIssues1831) {
	...
	while (1) {
		...
		ZEPHIR_CALL_FUNCTION(&info, "array_shift", &_1, 66, &_0$$3);
		ZEPHIR_UNREF(&_0$$3);
		zephir_check_call_status();
		ZEPHIR_INIT_NVAR(&_2$$3);
		ZVAL_STRING(&_2$$3, "header");
		ZEPHIR_CALL_FUNCTION(&_3$$3, "stripos", &_4, 67, &info, &_2$$3);
		zephir_check_call_status();
		if (!ZEPHIR_IS_FALSE_IDENTICAL(&_3$$3)) {
			...
			ZEPHIR_CALL_FUNCTION(NULL, "array_unshift", &_6, 68, &_5$$5, &info);
			ZEPHIR_UNREF(&_5$$5);
			zephir_check_call_status();
			break;
		}
...
zend_object *zephir_init_properties_Test_Properties_PropertyArray(zend_class_entry *class_type TSRMLS_DC) {
	...
	
	{
		...
		if (Z_TYPE_P(&_0) == IS_NULL) {
			...
			array_init(&_1$$3);
			zephir_init_property_zval(this_ptr, ZEND_STRL("otherArray"), &_1$$3);
		}
		...
		if (Z_TYPE_P(&_2) == IS_NULL) {
			...
			ZVAL_LONG(&_4$$4, 4);
			zephir_array_fast_append(&_3$$4, &_4$$4);
			zephir_init_property_zval(this_ptr, ZEND_STRL("someArray"), &_3$$4);
		}
		zephir_read_property(&_5, this_ptr, SL("someEmptyArray"), PH_NOISY_CC | PH_READONLY);
		if (Z_TYPE_P(&_5) == IS_NULL) {
			ZEPHIR_INIT_VAR(&_6$$5);
			array_init(&_6$$5);
			zephir_init_property_zval(this_ptr, ZEND_STRL("someEmptyArray"), &_6$$5);
		}
		ZEPHIR_MM_RESTORE();
