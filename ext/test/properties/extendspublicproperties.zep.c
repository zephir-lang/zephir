...
ZEPHIR_INIT_CLASS(Test_Properties_ExtendsPublicProperties) {
	...
	zend_declare_property_long(test_properties_extendspublicproperties_ce, SL("testPropertyFromClassNameConstantValue"), 123, ZEND_ACC_PUBLIC);

	zend_declare_property_null(test_properties_extendspublicproperties_ce, SL("someGetterSetterArray"), ZEND_ACC_PROTECTED);
...
zend_object *zephir_init_properties_Test_Properties_ExtendsPublicProperties(zend_class_entry *class_type TSRMLS_DC) {
	...
	
	{
		...
		if (Z_TYPE_P(&_0) == IS_NULL) {
			...
			zephir_init_property_zval(this_ptr, ZEND_STRL("someArrayWithSubArray"), &_1$$3);
		}
		...
		if (Z_TYPE_P(&_3) == IS_NULL) {
			...
			zephir_init_property_zval(this_ptr, ZEND_STRL("someArrayWithSpecifyKeys"), &_4$$4);
		}
		...
		if (Z_TYPE_P(&_5) == IS_NULL) {
			...
			ZVAL_LONG(&_7$$5, 4);
			zephir_array_fast_append(&_6$$5, &_7$$5);
			zephir_init_property_zval(this_ptr, ZEND_STRL("someArray"), &_6$$5);
		}
		zephir_read_property(&_8, this_ptr, SL("someEmptyArray"), PH_NOISY_CC | PH_READONLY);
		if (Z_TYPE_P(&_8) == IS_NULL) {
			ZEPHIR_INIT_VAR(&_9$$6);
			array_init(&_9$$6);
			zephir_init_property_zval(this_ptr, ZEND_STRL("someEmptyArray"), &_9$$6);
		}
		...
		if (Z_TYPE_P(&_10) == IS_NULL) {
			...
			zephir_init_property_zval(this_ptr, ZEND_STRL("someGetterSetterArray"), &_11$$7);
		}
		ZEPHIR_MM_RESTORE();
